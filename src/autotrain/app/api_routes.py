...
from autotrain.neuroflow import compile_pipeline as nf_compile_pipeline
from autotrain.neuroflow import run_pipeline as nf_run_pipeline
from autotrain.neuroflow.exceptions import NeuroFlowError
from autotrain.neuroflow.schemas import CompileResponse, PipelineGraph
...
LOG_FILE = Path(os.environ.get("AUTOTRAIN_LOGFILE", Path(__file__).resolve().parents[2] / "autotrain.log"))
...
# these are the apis for amlost all the taks for training
...
def _build_compile_response(artifacts, job_id: Optional[str] = None, outputs: Optional[Dict[str, Any]] = None) -> CompileResponse:
    return CompileResponse(
        config=artifacts.config,
        cli_command=artifacts.cli_command,
        python_code=artifacts.python_code,
        project_name=artifacts.project_name,
        task=artifacts.autotrain_task,
        base_model=artifacts.base_model,
        backend=artifacts.backend,
        hub_repo=artifacts.hub_repo,
        job_id=job_id,
        outputs=outputs,
    )
...
@api_router.post("/neuroflow/compile", response_model=CompileResponse)
async def api_neuroflow_compile(graph: PipelineGraph):
    try:
        artifacts = nf_compile_pipeline(graph)
    except NeuroFlowError as exc:
        logger.error(f"NeuroFlow compile failed: {exc}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    return _build_compile_response(artifacts)


@api_router.post("/neuroflow/run", response_model=CompileResponse)
async def api_neuroflow_run(graph: PipelineGraph, request: Request):
    token = _extract_bearer_token(request)
    try:
        result = nf_run_pipeline(graph, token=token, username_hint=graph.username_hint)
    except NeuroFlowError as exc:
        logger.error(f"NeuroFlow run failed: {exc}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    return _build_compile_response(result.compilation, job_id=result.job_id, outputs=result.outputs)


def _read_logs(limit: int = 200, project_name: Optional[str] = None) -> List[str]:
    if not LOG_FILE.exists():
        return []
    try:
        lines = LOG_FILE.read_text(encoding="utf-8", errors="ignore").splitlines()
    except Exception as exc:  # pragma: no cover - best-effort log read
        logger.error(f"Failed to read log file: {exc}")
        return []
    if project_name:
        lines = [line for line in lines if project_name in line]
    return lines[-limit:]


@api_router.get("/neuroflow/logs", response_class=JSONResponse)
async def api_neuroflow_logs(project_name: Optional[str] = None, limit: int = 200):
    try:
        log_lines = _read_logs(limit=limit, project_name=project_name)
    except Exception as exc:
        logger.error(f"Failed to gather logs: {exc}")
        raise HTTPException(status_code=500, detail="Unable to read logs") from exc
    return {"logs": log_lines, "project_name": project_name}


@api_router.post("/neuroflow/upload", response_class=JSONResponse)
async def api_neuroflow_upload(file: UploadFile = File(...)):
    try:
        data_dir = Path("data")
        ...
        return {"filename": filename, "path": str(file_path.absolute()), "success": True}
    except Exception as exc:
