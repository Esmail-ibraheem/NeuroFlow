...
try:
    import pandas as pd
except Exception as pandas_import_error:  # pragma: no cover - optional dependency
    pd = None
    _PANDAS_ERROR = pandas_import_error
else:
    _PANDAS_ERROR = None
...
def _ensure_pandas():
    if pd is None:
        raise RuntimeError(
            "pandas (and its compiled dependencies such as numpy/pyarrow) are required for tabular/text datasets."
        ) from _PANDAS_ERROR

from preprocessor.dreambooth import DreamboothPreprocessor
from preprocessor.tabular import (
...
from preprocessor.vision import (
    ImageClassificationPreprocessor,
    ImageRegressionPreprocessor,
    ObjectDetectionPreprocessor,
)
from preprocessor.vlm import VLMPreprocessor
