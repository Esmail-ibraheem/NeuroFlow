...
def process_input_data(config):
    ...
    elif getattr(config, "train_test_split", None) is not None:
        logger.info(f"Splitting train data into train and validation with test_size={config.train_test_split}")
        dataset = train_data.train_test_split(test_size=config.train_test_split, seed=config.seed)
